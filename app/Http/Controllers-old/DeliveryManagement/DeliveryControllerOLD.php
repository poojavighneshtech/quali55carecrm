   ...
   public function ModifyDeliveryPost(Request $request)
   {
	...
	if($isLoggedIn == 'false')
	{
	  ...
	  foreach($update_data as $key => $value)
	  {
		...
		$old_order_details = DB::table('order_details')->select('id','creation_date','months','pickup_date')->where('order_id', $order_id)->get();
   ...
   public function filterPickupOrderDWS()
   {
	...
	{    
		...
		for($i=0; $i<count($data['product_details']); $i++)
		{
			...
			$get_warehouse_details = DB::select("SELECT * FROM vendor_warehouse WHERE vendor_id = 17 AND id IN (19,175)");
			$get_warehouse_details =json_decode(json_encode($get_warehouse_details), true);
	...
	public function assign_pickup_delboy($order_id)
	{
		...
		{			
			...
			$get_warehouse_details = DB::table('vendor_warehouse')->where('vendor_id',17)->whereIn('id',[19,175])->get()->toArray();
			...
			if(DB::table('adjustment_table')->where('adjusted_order_details_id',$value1->id)->exists())
			{
				// dd("Exists");
				$records = DB::table('adjustment_table')->select('adjusted_amount')->where('adjusted_order_details_id',$value1->id)->where('fromtype','D')->get();
				// dd($records);
    ...
    public function assign_pickup_delboy_post(Request $request)
    {
        ...
        if($pickup_order_id!=null)
        {
			...
			DB::table('del_orders')->where('order_id',$pickup_order_id)->update(['TotalAmt'=>$pickup_sum,'comment'=>implode(',',$comment)]);
