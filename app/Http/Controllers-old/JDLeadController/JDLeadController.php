...
class JDLeadController extends Controller
{
   ...
   public function viewAllJDLeads()
   {
      ...
      $jd_leads = DB::select("SELECT * FROM jd_leads WHERE status = 'New' AND jd_leads.date >= '$past_three_days' ORDER BY jd_leads.jd_leads_id DESC");
   ...
   public function viewAllInProcessLeads()
   {
      ...
      $date = date('Y-m-d');
      $past_three_days = date('Y-m-d',strtotime("-2 days"));
      if(session('role') == "superuser" OR session('role') == "admin")
      {
         $jd_leads = DB::select("SELECT * FROM jd_leads where status='In Process' AND jd_leads.date >= '$past_three_days' ORDER BY jd_leads.jd_leads_id DESC");
      }
      elseif(session('role') == 'user')
      {
         $session_user_id = session('user_id');
         $jd_leads = DB::select("SELECT * FROM jd_leads where status='In Process' AND jd_leads.date >= '$past_three_days' AND lead_owner = $session_user_id ORDER BY jd_leads.jd_leads_id DESC");
      }
      $data['jd_lead_details'] = json_decode(json_encode($jd_leads), true);
   ...
   public function viewAllClosedLeads()
   {
      ...
      if(session('role') == "superuser" OR session('role') == "admin")
      {
         $jd_leads = DB::select("SELECT * FROM jd_leads where (status!='New' AND status!='In Process' AND status!='Converted') AND jd_leads.date >= '$past_three_days' ORDER BY jd_leads.jd_leads_id DESC");
      }
      elseif(session('role') == 'user')
      {
         $session_user_id = session('user_id');
         $jd_leads = DB::select("SELECT * FROM jd_leads where (status!='New' AND status!='In Process' AND status!='Converted') AND jd_leads.date >= '$past_three_days' AND lead_owner = $session_user_id ORDER BY jd_leads.jd_leads_id DESC");
      }
      $data['jd_lead_details'] = json_decode(json_encode($jd_leads), true);
      for ($i=0; $i <count($data['jd_lead_details']) ; $i++) { 
         $user_id = $data['jd_lead_details'][$i]['lead_owner'];
...
public function in_process($jd_lead_id, $user_id)
{
   ...
   $jd_leads = DB::select("SELECT * FROM jd_leads where jd_leads_id = '$jd_lead_id' ");
   $data['jd_lead_details'] = json_decode(json_encode($jd_leads), true);
   //$JD_Leads->where('jd_leads_id',$jd_lead_id)->update($lead_status);
   return view('JDLead/view_jd_lead',$data);
}
public function progress_jd_lead()
{
   $isLoggedIn = $this->isLoggedIn();
   if($isLoggedIn == 'false')
   {
   ...
   $JD_Leads->where('jd_leads_id',$jd_lead_id)->update($lead_info);
   return redirect('/view_all_inprocess_leads')->with('message', 'Lead Updated Successfully');
}
   ...
   //-----view jd_lead ====//
   public function jd_view_lead($customer_id,$id)
   {
      $isLoggedIn = $this->isLoggedIn();
      if($isLoggedIn == 'false')
      {
      ...
      //print_r($data);
      return view('JDLead/view_lead',$data);
   }
   //----------Q5c leads ------//
   public function viewAllQ5CLeads()
   {
      ...
      if(session('role') == "superuser" OR session('role') == "admin")
      {
         $leads = DB::select("SELECT * FROM leads,customer_details WHERE leads.customer_id = customer_details.cust_id AND leads.lead_status = 'Quali5Care' AND leads.creation_date >= '$past_three_days' ORDER BY leads.creation_date DESC");
      }
      elseif(session('role') == "user")
      {
         $session_user_id = session('user_id');
         $leads = DB::select("SELECT * FROM leads,customer_details WHERE leads.customer_id = customer_details.cust_id AND leads.lead_status = 'Quali5Care' AND leads.creation_date = '$past_three_days' AND leads.lead_owner = $session_user_id ORDER BY leads.creation_date DESC");
      }
   ...
   public function filterJDLeadsViewAll($filter_by)
   {
      ...
      $jd_leads = DB::select("SELECT * FROM jd_leads WHERE status = 'New' AND $whereClause ORDER BY jd_leads.jd_leads_id DESC");
      $data['jd_lead_details'] = json_decode(json_encode($jd_leads), true);
      return view('JDLead/view_all_jd_lead',$data);
   }
