...
class EditOrderController extends Controller
{
    ...
    public function updateOrderProduct(Request $request)
    {
        ...
        {
            $vendor_product_id = $request->get("batch_id");
            $warehouse_id = $request->get("warehouse_id");
            $product_type = $request->get("product_type");
            $data['inventory_details'] = $this->getInventory($vendor_product_id,$warehouse_id);
            return $data;
        }
        else if($request->get("request_type") == "update-data-order")
        {
            ...
            if($product_type == "Rental")
            {
                ...
                {
                    $updateData['vendor_product_details_id'] = $inventory;
                    ...
                    array_push($updatedDataActivityLog['new_value'],$inventory);
                    if($order_details[0]->status == "Accepted")
                    {
                        // echo $inventory;
                        // $inventory_id = VendorProductDetails::where('id',$inventory)->first('inventory_id');
                        $inventory_id = DB::table('vendor_product_details')->select('inventory_id')->where('id',$inventory)->get();
                        // if($order_details[0]->product_batch != $batch)
                        // {
                            $updateData['unique_id'] = $inventory_id[0]->inventory_id;
                            VendorRentedProducts::where('id',$order_details[0]->rented_product_id)->update(['unique_id'=>$inventory_id[0]->inventory_id, 'vendor_product_id'=>$batch]);
                    ...
                    $order_detail = DB::table('customer_details')->join('order_details','order_details.customer_id','=','customer_details.cust_id')->select('customer_details.city','order_details.*')->where('order_details.id',$order_details_id)->first();
                    DB::table('vendor_rented_inventory')->where('order_details_id',$order_details_id)->where('type','Delivery')->update(['flag'=>'Inactive']);
                    ...
                    VendorProductDetails::where('id',$inventory)->update(['availability_status'=>1,'current_location'=>0]);
                }
            }
        ...
        else if($request->get("request_type") == "remove-product")
        {
            ...
            $vendor_product_details_id = $order_details[0]->vendor_product_details_id;
            ...
            ActivityLog::insert([
                ...
                'fields'=>'Product_id',
                'old_value'=>$order_details[0]->product_id,
                'new_value'=>'-',
                'updated_by'=>session('username')
            ]);
            ...
            $customer_name = DB::table('customer_details')->where('cust_id',$order_details[0]->customer_id)->first();
            $customer_name = $customer_name->customer_name;
            $accountsEmail = config('app.accounts_email');
            //$accountsEmail = 'viveks@quali55care.com';
            $orderType = 'Delivery';
            ...
            {
                $product_details .= "Product Name: ".$product->product_name;
                $product_details .= " | Rent/Sale: ".$product->product_rent;
                $product_details .= " | Deposit: ".$product->product_deposite;
                $product_details .= " | Transport: ".$product->transport;
                $product_details .= " | Type: ".$product->sale_rental;
                $product_details .= " | Vendor: ".$product->vendor_name;
            }
            if($order_details[0]->product_rent !=0)
            {
                DB::table('cr_dr_note')->insert([
                    'order_id'=>$order_details[0]->order_id,
                    'order_details_id'=>$order_details[0]->id,
                    'crdrtype'=>'Cr',
                    'intype'=>'R',
                    'amount'=>($order_details[0]->product_rent),
                    'createdby'=>session('username')
                ]);
            }
            if($order_details[0]->product_deposite !=0)
            {
                DB::table('cr_dr_note')->insert([
                    'order_id'=>$order_details[0]->order_id,
                    'order_details_id'=>$order_details[0]->id,
                    'crdrtype'=>'Cr',
                    'intype'=>'D',
                    'amount'=>($order_details[0]->product_deposite),
                    'createdby'=>session('username')
                ]);
            }
            if($order_details[0]->transport !=0)
            {
                DB::table('cr_dr_note')->insert([
                    'order_id'=>$order_details[0]->order_id,
                    'order_details_id'=>$order_details[0]->id,
                    'crdrtype'=>'Cr',
                    'intype'=>'T',
                    'amount'=>($order_details[0]->transport),
                    'createdby'=>session('username')
                ]);
            }
        ...
        else if($request->get("request_type") == "fetch-order-product-details")
        {
            ...
            {
                if(DB::table('adjustment_table')->where('order_details_id',$request->get('order_details_id'))->where('adjusted_order_details_id',$value->id)->exists())
                {
                    $adjusted_record = DB::table('adjustment_table')->where('order_details_id',$request->get('order_details_id'))->where('adjusted_order_details_id',$value->id)->first();
        ...
        else if($request->get("request_type") == "adjust-deposit")
        {
            ...
            foreach($request->get('adjusted_deposit') as $key=>$value)
            {
                ...
                if(DB::table('adjustment_table')->where('order_details_id',$request->get('act_order_details_id'))->where('adjusted_order_details_id',$request->get('hidden_adjusted_order_details_id')[$key])->exists())
                {
    ...
    public function fetchOrderProductExists($order_details_id)
    {
        ...
        $inventory_details = $this->getInventory($order_details[0]->vendor_product_id,$order_details[0]->vendor_warehouse_id);
    ...
    public function getVendor($product_id,$product_qty,$product_type)
    {
        if($product_type == 'Rental')
        {
            $vendors = DB::select("SELECT DISTINCT vendor_details.id as vendor_id, vendor_details.registered_name as vendor_name FROM vendor_details, vendor_products WHERE vendor_products.product_id = $product_id AND vendor_products.vendor_id = vendor_details.id AND vendor_products.product_quantity >= $product_qty AND (vendor_products.status != 'Pending' OR vendor_products.status != 'Rejected')");
            return $vendors;
        }
        else if($product_type == 'Sale')
        {
    ...
    public function getInventory($vendor_product_id,$warehouse_id)
    {
        $inventory = DB::select("SELECT vendor_product_details.id as vendor_product_details_id, vendor_product_details.inventory_id as inventory_id FROM vendor_product_details WHERE vendor_products_id = $vendor_product_id AND warehouse_id = $warehouse_id AND availability_status=0 AND current_location != 0");
        return $inventory;
    }
