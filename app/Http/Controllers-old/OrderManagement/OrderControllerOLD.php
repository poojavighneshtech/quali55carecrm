<?php
    ...
    class OrderController extends Controller
    {
                                ...
                                if($_POST['assign']=='Individual')
                                {
                                    ...
                                    foreach ($temp_order_details as $key=>$temp_order_detail)
                                    {
                                        ...
                                        for ($i=0; $i <count($temp_order_detail) ; $i++)
                                        {
                                            ...
                                            /**/ $order_details_id_11 = $order_details->insertGetId($insert_order);
                                            DB::table('vendor_rented_inventory')->insert(
                                                [
                                                    "vendor_id" => $temp_vendor_id,
                                                    "order_id" => $order_id,
                                                    "order_details_id" => $order_details_id_11,
                                                    "inventory_id" => $temp_vendor_product_details_id,
                                                    "vendor_product_id" => $temp_product_batch,
                                                    "rented_date" => $date,
        ...
        public function generate_delivery_invoice(Request $request)
        {
            ...
            $orderTypeNotIn = config('app.order_type');
            $order_data = DB::table('del_orders')->join('leads','leads.id','=','del_orders.lead_id')->select('leads.patient_name','del_orders.*')->where('lead_id',$request->get('lead_id'))->whereNotIn('status',['Closed'])->whereNotIn('del_orders.deliverypickup',$orderTypeNotIn)->get();
                ...
                foreach($value as $k=>$v)
                {
                    ...
                    if($key == 'Rental')
                    {
                        ...
                        else
                        {
                            ...
                            $product_details_rent[$index]['pickup_date'] = $v->pickup_date;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            $product_details_sale[$index]['pickup_date'] = $v->pickup_date;
        ...
        public function generate_renewal_invoice(Request $request)
        {
            ...
            $order_data = DB::table('del_orders')->join('leads','leads.id','=','del_orders.lead_id')->select('leads.patient_name','del_orders.*')->where('order_id',$request->get('order_id'))->whereNotIn('status',['Closed'])->get();
            $order_id = $order_data->pluck('order_id')->toArray();
            $data['order_date'] = $order_data[0]->DelDate;
        ...
        function delveryOrdersAll(Request $request){
            ...
            $get_all_leads = DB::table('leads')
                ...
                ->when($request->get('filter_patient_name'),function($query)use($request){
                    $query->where('leads.filter_patient_name','=',$request->get('filter_patient_name'));
                })
        ...
        public function renewalnotice(){
            ...
            array_push($order_details,"9370738471");
            foreach($order_details as $key=>$value){
