<?php
    ...
    class OrderController extends Controller
    {
        ...
        public function pending_for_vendor_approval()
        {
            ...
            $order_details = DB::select("SELECT * FROM del_orders WHERE  del_orders.order_approval_status='Pending' AND del_orders.status != 'Cancel' AND del_orders.DelDate ='$today' ");
            // echo "SELECT * FROM del_orders WHERE  del_orders.order_approval_status='Pending' AND del_orders.DelDate ='$today' ";
            $data['order_details'] = json_decode(json_encode($order_details),true);
            echo "<script>localStorage['filtered']='today';</script>";        
        ...
        public function ViewAllOrdersFilter(Request $request)
        {
            ...
            $form_max_date = date('d-m-Y',strtotime($get_max_date->DelDate));
                ...
                foreach($get_all_orders as $key=>$order)
                {
                    ...
                    if($del_status=='Delivery')
                    {
                        $get_product_details = DB::table('order_details')
                                            ...
                                            ->select('product_name')
                                            ->where('order_id',$get_order_id)->get()->pluck('product_name')->toArray();
                    }
            ...
            else
            {
                ...
                foreach($get_all_orders as $key=>$order)
                {
                    ...
                    $product_name_imp = implode(',',$get_product_details);
                    //$get_products['data'][$key]['product_name'] = $product_name_imp;
                    $products_arr[$key] = $product_name_imp;
                }
            }
