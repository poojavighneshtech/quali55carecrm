<?php
    ...
    class OrderController extends Controller
    {
        ...
        public function ViewAllLeads_new(Request $request)
        {
            ...
            $from_date = null;
            $end_date = null;//$request->get('filter_end_date');
        ...
        public function pending_for_vendor_approval()
        {
            ...
            $data['end_date'] = date('Y-m-d');
            $temp_order_details = DB::select("SELECT * FROM del_orders,order_details WHERE del_orders.order_id=order_details.order_id AND del_orders.order_approval_status='Pending' AND del_orders.DelDate='$today' ");
            ...
            $order_details = DB::select("SELECT * FROM del_orders WHERE  del_orders.order_approval_status='Pending' AND del_orders.DelDate ='$today' ");
        ...
        public function status_change()
        {
            ...
            for ($i=0; $i < count($vendor_warehouse_id); $i++) 
            {
                ...
                $status_order_details = DB::update("UPDATE order_details SET unique_id = '$unique_id', status = 'Accepted' WHERE order_id IN($temp_order_id)");
        ...
        public function filterPendingVendorApproval($filter_by)
        {
            ...
            elseif($filter_by =='all')
            {
                ...
                $temp_order_details = DB::select("SELECT * FROM del_orders,order_details WHERE del_orders.order_id=order_details.order_id AND del_orders.order_approval_status='Pending' ");
                $data['temp_order_details'] = json_decode(json_encode($temp_order_details),true);
                $prev_order_id = 0;
                $count_status = 1;
                $status = null;
                ...
                $order_details = DB::select("SELECT * FROM del_orders WHERE  del_orders.order_approval_status='Pending'");
                //echo "SELECT * FROM del_orders,order_details WHERE del_orders.order_id=order_details.order_id AND del_orders.order_approval_status";
                $data['order_details'] = json_decode(json_encode($order_details),true);        
                return view('/OrderManagement/pending_for_vendor_approval',$data);
            }
            ...
            $temp_order_details = DB::select("SELECT * FROM del_orders,order_details WHERE del_orders.order_id=order_details.order_id AND del_orders.order_approval_status='Pending' AND $whereClause ");
            $data['temp_order_details'] = json_decode(json_encode($temp_order_details),true);
            $prev_order_id = 0;
            $count_status = 1;
            $status = null;
            ...
            $order_details = DB::select("SELECT * FROM del_orders WHERE  del_orders.order_approval_status='Pending' AND $whereClause ");
            //echo "SELECT * FROM del_orders,order_details WHERE del_orders.order_id=order_details.order_id AND del_orders.order_approval_status";
            $data['order_details'] = json_decode(json_encode($order_details),true);        
            return view('/OrderManagement/pending_for_vendor_approval',$data);     
        }
        ...
        public function approve_orders()
        {
            ...
            for ($i=0; $i <count($order_checked) ; $i++)
            {
                ...
                for ($j=0; $j <count($data['order_details']) ; $j++) 
                {
                    ...
                    //order_details status changing to accpted
                    $status_order_details = DB::update("UPDATE order_details SET unique_id = '$unique_id', status = 'Accepted' WHERE order_id ='$order_id' AND id='$order_details_id' ");
        ...
        public function individual_vendor_batch($equipment,$eq_quantity)
        {
            $isLoggedIn = $this->isLoggedIn();
            if($isLoggedIn == 'false')
            {
            ...
            if($_SERVER['REQUEST_METHOD']=='GET')
            {
                $vendors = DB::select("SELECT DISTINCT vendor_details.id as vendor_id, vendor_details.registered_name as vendor_name FROM vendor_details, vendor_products WHERE vendor_products.product_id = $equipment AND vendor_products.vendor_id = vendor_details.id AND vendor_products.product_quantity >= $eq_quantity AND (vendor_products.status != 'Pending' OR vendor_products.status != 'Rejected')");
        ...
        public function select_product_brand($equipment,$vendor_id,$warehouse_id)
        {
            ...
            if($_SERVER['REQUEST_METHOD']=='GET')
            {
                $brands = DB::select("SELECT DISTINCT product_brands.id as brand_id, product_brands.brand_name as brand_name FROM product_brands,vendor_products WHERE vendor_products.product_id = $equipment AND vendor_products.vendor_id = $vendor_id AND vendor_products.warehouse_id = $warehouse_id AND product_brands.id = vendor_products.product_brand");
        ...
        public function select_batch($equipment,$vendor_id,$warehouse_id,$brand_id)
        {
            ...
            if($_SERVER['REQUEST_METHOD']=='GET')
            {
                $brands = DB::select("SELECT vendor_products.id as vendor_product_id, vendor_products.batch as batch_name, vendor_products.product_rent_approved as product_rent FROM vendor_products WHERE vendor_products.product_id = $equipment AND vendor_products.vendor_id = $vendor_id AND vendor_products.warehouse_id = $warehouse_id AND vendor_products.product_brand = $brand_id");
                $data['brands'] = json_decode(json_encode($brands), true);
                $jsonstring = json_encode($data['brands']);
                echo $jsonstring;
            }
        ...
        public function DateFilter($type)
        {
            ...
            elseif($type=='vendor_pending_approval')
            {
                ...
                $temp_order_details = DB::select("SELECT * FROM del_orders,order_details WHERE del_orders.order_id=order_details.order_id AND del_orders.order_approval_status='Pending' AND del_orders.DelDate BETWEEN '$start_date' AND '$end_date'");
                ...
                $order_details = DB::select("SELECT * FROM del_orders WHERE  del_orders.order_approval_status='Pending' AND del_orders.DelDate BETWEEN '$start_date' AND '$end_date'");
                $data['order_details'] = json_decode(json_encode($order_details),true);
                echo "<script>localStorage['filtered']='today';</script>";        
                return view('/OrderManagement/pending_for_vendor_approval',$data);
            }
        ...
        public function ViewAllOrdersFilter(Request $request)
        {
            $match_clause = array();
            $whereCondition = [];
            $get_min_date = DelOrders::min('DelDate');
            $form_min_date = date('d-m-Y',strtotime($get_min_date));
            $get_max_date = DelOrders::max('DelDate');
            ...
            if(isset($delivery_status) && ($delivery_status=='Pending' || $delivery_status=='Assigned' || $delivery_status=='Accepted' || $delivery_status=='InProgress' || $delivery_status=='Delivered')){
                $whereCondition5 = ['status','=',$delivery_status];
            ...
            {
                return view('OrderManagement/view_all_orders',compact('get_all_orders'),compact('filter_arr'));
            }
