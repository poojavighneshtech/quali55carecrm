...
class RenewalPickupController extends Controller
{
    ...
    public function renewal_pickup(Request $request)
    {
        ...
        if($_SERVER['REQUEST_METHOD']=='GET')
        {
            ...
            {
                if($request->get('user_filter') == 'all')
                {
                    $whereClause = "leads.lead_owner = user.id";
                }
                else
                {
                    $user_id_lead = $request->get('user_filter');
                    $whereClause = "leads.lead_owner = user.id AND user.role='user' AND user.id='$user_id_lead' ";
                }
            }
            else
            {
                ...
                {
                    // $whereClause = "leads.lead_owner = user.id AND user.role='user' AND user.id='$user_id' ";
                    $whereClause = "leads.lead_owner = user.id";
                }
                if(session('role')=='admin')
                {
                //    $whereClause = "leads.lead_owner = user.id AND user.role='admin' AND user.id='$user_id' ";
                   $whereClause = "leads.lead_owner = user.id";
                }
            }
            ...
            $whereDate = "order_details.pickup_date='$date'";
            $LocalStorageDate = "today";
            //for export excel
            $exp_current_value = "Today";
            if(!empty($request->get('date_filter')))
            {
                ...
                elseif($request->get('date_filter')=='tommorow'){
                    $date = date('Y-m-d',strtotime("+1 days"));
                    $whereDate = "order_details.pickup_date='$date'";
                    $LocalStorageDate = "tommorow";
                    $exp_current_value = "Tommorow";
                }
                elseif($request->get('date_filter')=='overdue'){
                    $whereDate = "order_details.pickup_date<'$date'";
                    $LocalStorageDate = "overdue";
                    $exp_current_value = "Overdue";
                }
                elseif($request->get('date_filter')=='3_days'){
                    $start_date = date('Y-m-d',strtotime("+1 days"));
                    $end_date = date('Y-m-d',strtotime("+3 days"));
                    $whereDate = "order_details.pickup_date BETWEEN '$start_date' AND '$end_date'";
                    $LocalStorageDate = "3_days";
                    $exp_current_value = "3_days";
                }
                elseif($request->get('date_filter')=='all'){
                    $get_min_date = OrderDetails::first('pickup_date');
                    $min_date = $get_min_date->pickup_date;
                    $max_date = Carbon::now()->toDateString();
                    $whereDate = "order_details.pickup_date BETWEEN '$min_date' AND '$max_date'";
                    $LocalStorageDate = "all";
                    $exp_current_value = "All";
                }
            }
            $whereClauseSearch = '';
            $btn_search = $request->get('btn_search');
            if(!empty($btn_search) && $btn_search=='date_search')
            {
                $exp_current_value = "Datewise";
                $start_date = $request->get('filter_start_date');
                $end_date = $request->get('filter_end_date');
                $whereDate = "order_details.pickup_date BETWEEN '$start_date' AND '$end_date'";
            }
            elseif(!empty($btn_search) && $btn_search=='customer_search')
            {
                $exp_current_value = "Textwise";
                $cust_no_name = $request->get('text_search_customer');
                $whereClauseSearch = "AND customer_details.customer_name LIKE '%$cust_no_name%' OR primary_contact_no LIKE '%$cust_no_name%'";
            }
            ...
            $renewal_pickup_info = DB::select("SELECT 
                        ...
                        where customer_details.cust_id = order_details.customer_id
                            ...
                            AND(order_details.current_status='Pending'
                                ...
                                OR order_details.current_status='CustStop')
                            AND $whereDate
            ...
            foreach($data['renewal_pickup_info'] as $renewal_pickup_info)
            {
                ...
                if(in_array($renewal_pickup_info['customer_id'],$cust_id_array))
                {
                    ...
                    for($i=0; $i<count($customer_products_details); $i++)
                    {
                        ...
                        if($customer_products_details[$i]['customer_id'] == $renewal_pickup_info['customer_id'])
                        {
                            ...
                            $customer_products_details[$i]['product_details'][$count]['upgraded'] = $renewal_pickup_info['upgraded'];
                            ...
                            $customer_products_details[$i]['product_details'][$count]['unique_id'] = $renewal_pickup_info['unique_id'];
                            ...
                            if($temp_product_quantity>1)
                            {
                                ...
                                { 
                                    ...
                                    $quantity_product[$j]['upgraded'] = $renewal_pickup_info['upgraded'];
                                    ...
                                    $quantity_product[$j]['unique_id'] = $renewal_pickup_info['unique_id'];
                ...
                else
                {
                    ...
                    $customer_products_details[$count]['product_details'][0]['upgraded'] = $renewal_pickup_info['upgraded'];
                    ...
                    $customer_products_details[$count]['product_details'][0]['unique_id'] = $renewal_pickup_info['unique_id'];
            ...
            $customer_products_details = $this->paginate($customer_products_details,10,$page);
            //dd($customer_products_details);
            echo "<script>localStorage['filtered']='$LocalStorageDate';</script>";
            //show total stops orders
            $getStops = DB::table('order_details')
                        ->select('product_id','product_name')
                        ->join('products','order_details.product_id','=','products.id')
                        ->where([['current_status','=','CustStop']])->get();
            $getProductwiseData = $getStops->groupBy('product_id')->toArray();
                        //dd($getStops,$getProductwiseData);
            return view('RenewalPickup/renewal_pickup',compact('customer_products_details','total_customer','total_equipment','total_due_amount','exp_current_value','getStops','getProductwiseData'));
        }
    }
    //paginate apply
    ...
    public function renewal_pickup_filter($filter,$cust_id)
    {
       ...
       //user based
        ...
        if(session('role')=='user')
        {
            //$role = "leads.lead_owner = user.id AND user.role='user' AND user.id='$user_id' ";
            $role = "leads.lead_owner = user.id";
        }
        if(session('role')=='admin')
        {
            //$role = "leads.lead_owner = user.id AND user.role='admin' AND user.id='$user_id' ";
            $role = "leads.lead_owner = user.id";
        }
        if(session('role')=='superuser')
        {
        ...
        if($filter=="today")
        {
            ...
            if($cust_id!="All")
            {
                $whereClause = "AND $role AND order_details.customer_id = $cust_id AND order_details.customer_id = customer_details.cust_id AND order_details.pickup_date ='$date' ";
            }
            else{
                $whereClause = "AND $role AND order_details.customer_id = customer_details.cust_id AND order_details.pickup_date ='$date' ";
            }
        ...
        elseif($filter=="3_days")
        {
            ...
            if($cust_id!="All")
            {
                $whereClause = "AND $role AND order_details.customer_id = $cust_id AND order_details.customer_id = customer_details.cust_id AND order_details.pickup_date BETWEEN'$date' AND '$end_date' ";
                
            }
            else{

                $whereClause = "AND $role AND order_details.customer_id = customer_details.cust_id AND order_details.pickup_date BETWEEN'$date' AND '$end_date' ";
            }
        }
        elseif($filter == "all")
        {
        ...
        $renewal_pickup_info = DB::select("SELECT 
                                                ...
                                                where order_details.order_id=del_orders.order_id
                                                    ...
                                                    AND (order_details.current_status='Pending'
                                                        ...
                                                        OR order_details.current_status='CustStop')
                                                    AND del_orders.deliverypickup NOT IN ($orderTypeNotIn)
                                                    $whereClause ORDER BY order_details.pickup_date ASC");
            $data['renewal_pickup_info'] = json_decode(json_encode($renewal_pickup_info),true);
            // print_r($data['renewal_pickup_info']);
            $cust_id_array = array();
            $customer_products_details = array();
    ...
    public function renewal_pickup_search(Request $request)
    {
        $isLoggedIn = $this->isLoggedIn();
        if($isLoggedIn == 'false')
        {
            ...
            $renewal_pickup_info = DB::select("SELECT 
                                                ...
                                                where 
                                                    ...
                                                    AND (order_details.current_status='Pending'
                                                        ...
                                                        OR order_details.current_status='CustStop')
                                                    AND del_orders.deliverypickup NOT IN ($orderTypeNotIn)
                                                    AND order_details.pickup_date BETWEEN '$start_date' AND '$end_date'
                                                    ORDER BY order_details.pickup_date ASC");
            
            $data['renewal_pickup_info'] = json_decode(json_encode($renewal_pickup_info),true);
        }
        elseif($btn=='customer_search')
        {
            //echo "df";
            $text_customer = $_POST['text_search_customer'];
            $data['text_customer'] = $text_customer;
            $cust_val = $_POST['text_search_customer'];
            $patient_val = "";
            $address_val = "";
            $exp_current_value = "Textwise";
            $orderTypeNotIn = config('app.order_type');
            ...
            $renewal_pickup_info = DB::select("SELECT 
                                                ...
                                                order_details,customer_details,del_orders,products,leads,user,vendor_details
                                                where 
                                                    (customer_details.customer_name LIKE '%$text_customer%'
                                                    OR customer_details.primary_contact_no LIKE '%$text_customer%'
                                                    OR customer_details.address_line_1 LIKE '%$text_customer%'
                                                    OR customer_details.address_line_2 LIKE '%$text_customer%'
                                                    OR customer_details.area LIKE '%$text_customer%'
                                                    OR customer_details.location LIKE '%$text_customer%'
                                                    OR customer_details.landmark LIKE '%$text_customer%'
                                                    OR leads.patient_name LIKE '%$text_customer%')
        ...
        elseif($btn=='address_search')
        {
            //echo "df";
            $exp_current_value = "AddressSearch";
            $text_address = $_POST['text_search_address'];
            $data['text_address'] = $text_address;
            $address_val = $_POST['text_search_address'];
            $patient_val = "";
            $cust_val = "";
            $exp_current_value = "AddressSearch";
            $orderTypeNotIn = config('app.order_type');
            ...
            $renewal_pickup_info = DB::select("SELECT 
                                                ...
                                                FROM
                                                    order_details,customer_details,del_orders,products,leads,user,vendor_details
                                                where 
                                                    (customer_details.address_line_1 LIKE '%$text_address%'
                                                    OR customer_details.address_line_2 LIKE '%$text_address%'
                                                    OR customer_details.location LIKE '%$text_address%'
                                                    OR customer_details.landmark LIKE '%$text_address%'
                                                    OR customer_details.area LIKE '%$text_address%')
                                                    AND order_details.customer_id = customer_details.cust_id
                                                    AND order_details.order_id=del_orders.order_id
                                                    AND order_details.product_id=products.id
                                                    AND del_orders.lead_id = leads.id
        ...
        elseif($btn=='patient_search')
        {
            $exp_current_value = "PatientSearch";
            $text_patient = $_POST['text_search_patient'];
            $data['text_patient'] = $text_patient;
            $cust_val = "";
            $patient_val = $_POST['text_search_patient'];
            $address_val = "";
            $orderTypeNotIn = config('app.order_type');
            $orderTypeNotIn ="'".implode("','",$orderTypeNotIn)."'";
            $renewal_pickup_info = DB::select("SELECT 
                                                ...
                                                FROM
                                                    order_details,customer_details,del_orders,products,leads,user,vendor_details
                                                where 
                                                    leads.patient_name LIKE '%$text_patient%'
                                                    AND order_details.customer_id = customer_details.cust_id
                                                    AND order_details.order_id=del_orders.order_id
                                                    AND order_details.product_id=products.id
                                                    AND del_orders.lead_id = leads.id
        ...
        elseif($btn=='stop_order_search'){
            // $start_date = $_POST['start_date'];
            // $end_date = $_POST['end_date'];
            // $data['start_date']= $start_date;
            // $data['end_date']= $end_date;
            $exp_current_value = "CustStop";
            $orderTypeNotIn = config('app.order_type');
            $orderTypeNotIn ="'".implode("','",$orderTypeNotIn)."'";
            $renewal_pickup_info = DB::select("SELECT 
                                                    order_details.id as order_details_id,
        ...
        elseif($btn=="stop_product_search"  && $request->method()=="GET"){
            $exp_current_value = "StopProductSearch";
        ...
        foreach($data['renewal_pickup_info'] as $renewal_pickup_info)
        {
            ...
            else
            {
                ...
                $customer_products_details[$count]['customer_log'] = $renewal_pickup_info['comment'];
                $customer_products_details[$count]['customer_address'] = $customer_address;
                $customer_products_details[$count]['isUpgraded'] = $renewal_pickup_info['isUpgraded'];
                $customer_products_details[$count]['product_details'][0]['product_name'] = $prod_name;
                $customer_products_details[$count]['product_details'][0]['upgraded'] = $renewal_pickup_info['upgraded'];
                $customer_products_details[$count]['product_details'][0]['vendor_name'] = $renewal_pickup_info['vendor_name'];
                $customer_products_details[$count]['product_details'][0]['pickup_date'] = $renewal_pickup_info['pickup_date'];
        ...
        $customer_products_details = $this->paginate($customer_products_details,100,$page);
        //print_r($data);
        // $getStops = DB::table('order_details')->where([['current_status','=','CustStop']])->get();
        $getStops = DB::table('order_details')
                        ->select('product_id','product_name')
                        ->join('products','order_details.product_id','=','products.id')
                        ->where([['current_status','=','CustStop']])->get();
        $getProductwiseData = $getStops->groupBy('product_id')->toArray();
        return view('RenewalPickup/renewal_pickup',compact('customer_products_details','total_customer','total_equipment','total_due_amount','cust_val','exp_current_value','patient_val','address_val','getStops','getProductwiseData'));
    ...
    public function getOrderData(Request $request)
    {
        ...
        foreach ($getOrderData as $key => $orderData) 
        {
            ...
            $today = Carbon::today()->toDateString();
            $monthCount = Carbon::parse($orderData->pickup_date)->diffInMonths($today);
            //echo $monthCount;
            $currentRenewDate = Carbon::parse($orderData->pickup_date)->addMonths($monthCount);
            if(Carbon::parse($currentRenewDate)->diffInDays($today)>0){
                $monthCount = $monthCount+1;
            }
            if(Carbon::parse($orderData->pickup_date)->diffInDays($today)==0)
            {
                $monthCount = 1;
            }
    ...
    public function orderCall(Request $request){
        if($request->get('submit')=='renew'){
           ...
           $status = $this->OrderPickup($pickupData);
        ...
        }if($request->get('submit')=='renew-and-pickup'){
            ...
            if($status=='success'){
                ...
                return redirect()->back()->with('error','Something went wrong contact IT Team');
            }
        }
    }
