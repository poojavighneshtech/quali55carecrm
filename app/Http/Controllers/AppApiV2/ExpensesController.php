...
class ExpensesController extends Controller
{
    ...
    public function updateExpense(Request $request){
        ...
        try{
            ...
            DB::commit();
            $expense = DB::table('daily_expenses')->where('user_name',$request->get('defusername'))->where('exp_date',$request->get('expDate'))->first();
            $this->send_wp_msg($expense->id, $expense->exp_date, $expense->user_name, $expense->cash_received_from_customer, $expense->cash_from_office, $expense->transport, $expense->deposite_paid, $expense->expenses, $expense->labour, $expense->balance_cash, $expense->img_url);
    ...
    public function send_wp_msg($exp_id,$date , $del_boy, $cash_rec_from_cust, $cash_rec_from_off, $transport, $depo_returned, $expenses, $labour_charges, $balance_cash, $receipt)
	{
        if(config('app.app_env') == 'devweb'){
            ...
            $contact_no = config('app.business_head_contact');
