...
class CustomerViewController extends Controller
{
    ...
    public function transaction_history(Request $request){
        ...
        $orders = DB::table('del_orders')->join('leads','leads.id','=','del_orders.lead_id')->join('order_details','order_details.order_id','=','del_orders.order_id')->distinct('del_orders.order_id')->select('del_orders.order_id','del_orders.DelDate','del_orders.TotalAmt','leads.patient_name','del_orders.deliverypickup','del_orders.order_expense','del_orders.vendor_charges','del_orders.labour_charges')->where('order_details.customer_id',$request->get('cust_id'))->get();
        if($request->has('order_id') && $request->get('order_id')!=null){
            $orders = DB::table('del_orders')->join('leads','leads.id','=','del_orders.lead_id')
                    ...
                    ->select('del_orders.order_id','del_orders.DelDate','del_orders.TotalAmt','leads.patient_name','leads.customer_id','del_orders.deliverypickup','del_orders.order_expense','del_orders.vendor_charges','del_orders.labour_charges')
        ...
        foreach($orders as $key=>$value)
        {
            ...
            $renewals = DB::table('renewals')->join('del_orders','renewals.collection_order_id','=','del_orders.order_id')
                            ->distinct('del_orders.order_id')
                            ->select('del_orders.order_id','del_orders.DelDate','del_orders.TotalAmt','renewals.start_date','renewals.end_date','del_orders.order_expense','del_orders.vendor_charges','del_orders.labour_charges')
                            ->where('renewals.order_id',$value->order_id)->whereNotIn('del_orders.status',['Cancel'])->get();
            $renewals = $renewals->groupBy('order_id');
            $pickups = DB::table('pickups')->join('del_orders','pickups.pickup_order_id','=','del_orders.order_id')->distinct('del_orders.order_id')->select('del_orders.order_id','del_orders.DelDate','del_orders.TotalAmt','del_orders.order_expense','del_orders.vendor_charges','del_orders.labour_charges')->where('pickups.del_order_id',$value->order_id)->whereNotIn('del_orders.status',['Cancel'])->get();
