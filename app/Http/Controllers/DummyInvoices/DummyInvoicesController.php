            ...
            try
            {
                ...
                foreach($request->get('inv_equip') as $key=>$prod_id)
                {
                    ...
                    [
                        ...
                        'billing_period'=>$request->get('billingPeriod')[$key],
                        'billing_unit'=>$request->get('billingUnit')[$key],
    ...
    public function viewDummyInvoice(Request $request)
    {
        ...
        foreach($product_details as $key=>$value)
        {
            ...
            foreach($value as $k=>$v)
            {
                if($v->billing_unit == 'Week'){
                    $pickupDate = date('Y-m-d',strtotime("+$v->billing_period $v->billing_unit",strtotime($invoice_details->date)));
                }else if($v->billing_unit == 'Half Month'){                    
                    $pickupDate = date('Y-m-d',strtotime("+$v->billing_period Week",strtotime($invoice_details->date)));
                }else if($v->billing_unit == 'Days'){
                    $pickupDate = date('Y-m-d',strtotime("+$v->billing_period Days",strtotime($invoice_details->date)));
                }else{
                    $pickupDate = date('Y-m-d',strtotime("+$v->billing_period months",strtotime($invoice_details->date)));
                }
                if($key == 'Rental')
                {
                    ...
                    else
                    {
                        ...
                        $product_details_rent[$index]['billing_period'] = $v->billing_period;
                        $product_details_rent[$index]['billing_unit'] = $v->billing_unit;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        $product_details_sale[$index]['billing_period'] = $v->billing_period;
                        $product_details_sale[$index]['billing_unit'] = $v->billing_unit;
        ...
        foreach($product_details as $key=>$value)
        {
            if($value['sale_rental'] == 'Rental')
            {
                ...
                $product_details[$key]['amount'] = ((($value['product_rent'])/($value['gst_rent']+100))*100)*$value['product_qty']*$value['billing_period'];
                $product_details[$key]['amount_cal'] = ((($value['product_rent'])/($value['gst_rent']+100))*100)*$value['product_qty']*$value['billing_period'];
                if(in_array($value['hsn_sac_rent'],$temp_hsn_code))
                {
                    ...
                    if($hsn_code_details[$index]['gst'] == $value['gst_rent'])
                    {
                        ...
                        $hsn_code_details[$index]['ct_rate'] = $value['gst_rent']/2;
                        $hsn_code_details[$index]['ct_amount'] = ($product_details[$key]['product_rate'])*(($value['gst_rent']/2)/100)*$value['product_qty']*$value['billing_period'];    
                        $hsn_code_details[$index]['st_rate'] = $value['gst_rent']/2;
                        $hsn_code_details[$index]['st_amount'] = ($product_details[$key]['product_rate'])*(($value['gst_rent']/2)/100)*$value['product_qty']*$value['billing_period'];    
                        $hsn_code_details[$index]['i_rate'] = $value['gst_rent'];
                        $hsn_code_details[$index]['i_amount'] = ($product_details[$key]['product_rate'])*(($value['gst_rent'])/100)*$value['product_qty']*$value['billing_period'];    
                ...
                $product_details[$key]['product_rate'] = number_format(((($value['product_rent']*$value['billing_period'])/($value['gst_rent']+100))*100),2);
