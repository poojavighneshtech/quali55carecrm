...
class hotLeadsControllertest extends Controller
{
   ...
   public function in_process_hot_leads($hot_lead_id, $user_id)
   {
		...
		$mobile_no = '8552944963';
			$sbPostData = '';
			$Body = '';
			$Body .= "Incident Name : \n";
			$Body .= "Incident Details : \n";
			$Body .= "Incident Address1 : \n";
			$Body .= "Incident Address2 : \n";
			
			$sbPostData .= "&mobile=" . urlencode($mobile_no);
			$sbPostData .= "&msg=" . urlencode($Body);
		
			$sendSMSUri = "http://api.mpillar.in/wapp/api/send?apikey=890f611dd41d40979dc26a1a0d7a4bd1";
		
			$ch = curl_init($sendSMSUri);
			curl_setopt($ch, CURLOPT_POST, true);
			curl_setopt($ch, CURLOPT_POSTFIELDS, $sbPostData);
		...
		return redirect('view_hot_leads_in_process_leads_test');
   }
   ...
   public function update_lead()
   {
	  ...
	  if($_SERVER['REQUEST_METHOD']=='POST')
	  {
		 ...
		 ////////////////////--------------Send Sms---------------//////////////////////////////
			...
			if(config('app.app_env') == 'devweb')
			{
				$mobiles = config('app.developer_contact');
			}
