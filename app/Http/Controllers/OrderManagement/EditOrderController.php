    ...
    public function updateOrderProduct(Request $request)
    {
        try{
            ...
            else if($request->get("request_type") == "update-data-order")
            {
                ...
                $remaining_products = DB::table('order_details')->select('id','product_rent','product_deposite','transport','billing_period')->where('order_id',$order_id)->whereNotIn('current_status',['Cancel'])->get();
                // $orderDetailsIds = $remaining_products->pluck('id')->toArray();
                foreach($remaining_products as $key=>$remainingProduct){
                    $remaining_products[$key]->product_rent =  RenewalPickupController::fetchCrDrData($remainingProduct->id,'R')*$remainingProduct->billing_period;
