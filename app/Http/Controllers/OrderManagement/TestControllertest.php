<?php
    ...
    class TestControllertest extends Controller
    {
        ...
        public function vendor_product_detailsold(Request $request)
        {
            ...
            $filter_data['sale_rental'] = null;
            $filter_data['order_no'] = null;
            ...
            $customer_order = $request->get('order_no');
            if(isset($customer_order)) {
                $whereCondition2 = ['order_details.order_id','=',$customer_order];
                array_push($whereClause,$whereCondition2);
                $filter_data['order_no'] = $customer_order;
            }
            $dateArr = [];
            ...
            $sale_rental = $request->get('sale_rental');
            if(isset($sale_rental)){
                if($sale_rental == 'Rental')
                {
                    $sale_rental = 'Rental';
                    $filter_data['sale_rental'] = 'Rental';
                }
                elseif($sale_rental == 'Sale')
                {
                    $sale_rental = 'Sale';
                    $filter_data['sale_rental'] = 'Sale';
                }
                else
                {
                    $sale_rental = 'All';
                    $filter_data['sale_rental'] = 'All';
                }
            }
            else
            {
            ...
            $order_details = DB::table('order_details')
                                ...
                                ->when($sale_rental,function($query,$sale_rental){
                                    if($sale_rental != 'All')
                                    {
                                        $query->where('order_details.sale_rental',$sale_rental);
                                    }
                                    elseif($sale_rental == 'Sale')
                                    {
                                        $sale_rental = 'Sale';
            ...
            return view('OrderManagement.vendor_product_detailsold',compact('products','order_details','filter_data','count'));
        }
            ...
            if($request->method() == "GET")
            {
                ...
                $order_details = DB::table('order_details')
                                   ...
                                   ->whereNotIn('order_details.current_status',['Cancel'])
                                    ...
                                    //     $query->where('order_details.order_id',$request->get('order_no'));
                                    // })
