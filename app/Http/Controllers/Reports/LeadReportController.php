...
class LeadReportController extends Controller
{
   ...
   {
      $userWiseLeads = DB::table('leads')->when($request->get('filter'),function($query)use($request){
         if($request->get('filter') == 'today'){
            $query->where('leads.creation_date',Carbon::now()->toDateString());
         }
         elseif($request->get('filter') == 'yesterday'){
            $query->where('leads.creation_date',Carbon::now()->subDays(1)->toDateString());
         }
         elseif($request->get('filter') == 'past_3_days'){
            $query->whereBetween('leads.creation_date',[Carbon::now()->subDays(2)->toDateString(),Carbon::now()->toDateString()]);
         }
         elseif($request->get('filter') == 'week'){
            $query->whereBetween('leads.creation_date',[Carbon::now()->startOfWeek()->toDateString(),Carbon::now()->toDateString()]);
         }
         elseif($request->get('filter') == 'month'){
            $query->whereBetween('leads.creation_date',[Carbon::now()->startOfMonth()->toDateString(),Carbon::now()->toDateString()]);
         }
      })->get();
      $totalEquipCount = $userWiseLeads->count();
      $userWiseLeads = $userWiseLeads->groupBy('lead_owner');

      $userWiseNurLeads = DB::table('nursing_care')->when($request->get('filter'),function($query)use($request){
         if($request->get('filter') == 'today'){
            $query->where('nursing_care.lead_date',Carbon::now()->toDateString());
         }
         elseif($request->get('filter') == 'yesterday'){
            $query->where('nursing_care.lead_date',Carbon::now()->subDays(1)->toDateString());
         }
         elseif($request->get('filter') == 'past_3_days'){
            $query->whereBetween('nursing_care.lead_date',[Carbon::now()->subDays(2)->toDateString(),Carbon::now()->toDateString()]);
         }
         elseif($request->get('filter') == 'week'){
            $query->whereBetween('nursing_care.lead_date',[Carbon::now()->startOfWeek()->toDateString(),Carbon::now()->toDateString()]);
         }
         elseif($request->get('filter') == 'month'){
            $query->whereBetween('nursing_care.lead_date',[Carbon::now()->startOfMonth()->toDateString(),Carbon::now()->toDateString()]);
         }
      })->get();
      $totalNurseCount = $userWiseNurLeads->count();
      $userWiseNurLeads = $userWiseNurLeads->groupBy('lead_owner');
      
      $users = DB::table('user')->where('role','user')->where('flag','Active')->get();
      $details = array();
      foreach($users as $key=>$user)
      {
         $details[$key]['user'] = $user->username;
         $details[$key]['total_leads'] = 0;
         $details[$key]['process'] = 0;
         $details[$key]['converted'] = 0;
         $details[$key]['closed'] = 0;
         $details[$key]['nur_total'] = 0;
         $details[$key]['nur_process'] = 0;
         $details[$key]['nur_converted'] = 0;
         $details[$key]['nur_closed'] = 0;
         if(isset($userWiseLeads[$user->id])){
            $details[$key]['total_leads'] = $userWiseLeads[$user->id]->count();
            $details[$key]['process'] = $userWiseLeads[$user->id]->where('lead_status',"Work In Process")->count();
            $details[$key]['converted'] = $userWiseLeads[$user->id]->whereIn('lead_status',["Converted","Order Generated"])->count();
            $details[$key]['closed'] = $userWiseLeads[$user->id]->whereNotIn('lead_status',["Work In Process","Converted","Order Generated"])->count();
         }
         if(isset($userWiseNurLeads[$user->id])){
            $details[$key]['nur_total'] = $userWiseNurLeads[$user->id]->count();
            $details[$key]['nur_process'] = $userWiseNurLeads[$user->id]->where('status',"Process")->count();
            $details[$key]['nur_converted'] = $userWiseNurLeads[$user->id]->whereIn('status',["Live","Stopped"])->count();;
            $details[$key]['nur_closed'] = $userWiseNurLeads[$user->id]->where('status',"Cancelled")->count();;
         }
      }
      // dd($details);
      return view('Reports.lead_reports',compact('details','totalEquipCount','totalNurseCount'));
   }
