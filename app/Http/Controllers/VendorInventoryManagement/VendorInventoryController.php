    ...
    public function vendorReturnInventoryCreate(Request $request){
        ...
        if($request->get('submit') != 'create'){
            if($request->get('type')=='img_update'){
                $oldDetails = DB::table('vendor_inventory_mgmt')->where('id',$request->get('row_id'))->first();
            }else{
                $oldDetails = DB::table('vendor_inventory_mgmt')->where('id',$request->get('hide_record_id'))->first();
            }
    
            if($request->get('return_pickup_date')){
                if($request->get('return_pickup_date') != $oldDetails->inventory_pickup_date && $oldDetails->state == 'out'){
                    if($oldDetails->details_id != null){
                        DB::table('pickups')->where('id',$oldDetails->details_id)->update(['drop_location'=>'Q5C','drop_warehouse_id'=>19]);                
                        DB::table('activity_log')->insert([
                                'order_type'=>'PO',
                                'key_id'=>$oldDetails->details_id,
                                'operation'=>'Order Modified',
                                'fields'=>'drop_location',
                                'old_value'=>'Vendor',
                                'new_value'=>'Q5C',
                                'updated_by'=>session('username')
                        ]);
        ...
        if($request->get('type')=='img_update'){
            ...
            $oldDetails = DB::table('vendor_inventory_mgmt')->where('id',$request->get('row_id'))->first();
            if($request->get('return_pickup_date') != $oldDetails->inventory_pickup_date && $oldDetails->state == 'out'){
                DB::table('vendor_inventory_mgmt')->where('id',$request->get('row_id'))->update(['pickup_address'=>"Quali55Care Warehouse, Vidya Vihar, Mumbai",'inventory_pickup_date'=>$request->get('return_pickup_date')]);
            }
        ...
        if($productImage){
            foreach($productImage as $key=>$image)
            {
                ...
                // $fileName = $request->get('return_date')."-".$request->get('return_product')."-".$request->get('return_product')."-".$key.'.'.$image->getClientOriginalExtension();;
