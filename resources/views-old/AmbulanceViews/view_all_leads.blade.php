        ...
        <div class="leads">
            ...
            <div class="card">
                <div class="card-header"  style="background-color: #4a6fdc; color: white;" >
                    <center>All Ambulance Leads</center>
                </div> 
                <div class="card-body">
                    <div class="card" id="filter_card">
                        ...
                        <div class="card-body collapse @if(isset($filter_arr) && !array_filter($filter_arr)) hide @else show @endif" id="filter-collapse" aria-labelledby="heading-filter">
                            <form action="{{url('/')}}/view_all_ambulance_leads" method="GET" id="all_lab_leads_form">
                                @csrf
                                <div class="row">
                                    <div class="col-md-10">
                                        <div class="row">
                                            ...
                                            <div class="col-md-5">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-9">
                                                        {{-- <input type="date" name="filter_end_date" id="input_end_date" class="form-control" value="@if(isset($filter_arr['end_date'])){{$filter_arr['end_date']}}@endif"> --}}
                                                        <select class="select form-control selectpicker" name="filter_customer_pickup_location" id="select_filter_customer_pickup_location" title="Customer Location"
                                                            data-live-search="true" data-size="5">
                                                            <option value="All" selected>All</option>
                                                            @foreach($get_pickup_locations as $location)
                                                                <option value="{{$location->pickup_location}}" @if(isset($filter_arr['pickup_location']) && $location->pickup_location==$filter_arr['pickup_location'])selected @endif>{{$location->pickup_location}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-9">
                                                        {{-- <input type="date" name="filter_end_date" id="input_end_date" class="form-control" value="@if(isset($filter_arr['end_date'])){{$filter_arr['end_date']}}@endif"> --}}
                                                        <select class="select form-control selectpicker" name="filter_customer_drop_location" id="select_filter_customer_drop_location" title="Customer Location"
                                                            data-live-search="true" data-size="5">
                                                            <option value="All" selected>All</option>
                                                            @foreach($get_drop_locations as $location)
                                                                <option value="{{$location->drop_location}}" @if(isset($filter_arr['drop_location']) && $location->drop_location==$filter_arr['drop_location'])selected @endif>{{$location->drop_location}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="table table-responsive">
                                ...
                                <tbody>
                                    @foreach($get_all_leads as $key=> $lead) 
                                        <tr>
                                            ...
                                            <td>{{$lead->waiting_time}}</td>
                                            <td>{{$lead->service_from}}</td>
                                            <td>{{$lead->service_to}}</td>
                                            <td>{{$lead->ambulance_type}}</td>
                                            ...
                                            <td>
                                                @if($lead->status=='In Process')
                                                    ...
                                                    {{--modal convert--}}
                                                    <form action="{{url('/')}}/amb_convert_lead" method="post">
                                                        @csrf
                                                        <div class="modal fade" id="convert_modal" tabindex="-1" role="dialog" aria-labelledby="convert_modalLabel" aria-hidden="true">
                                                            ...
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    Are you sure want to convert lead
                                                                    <input type="hidden" name="lead_id" id="modal_txt_lead_id" value="{{$lead->amb_id}}">
                                                                </div>
                                                    ...
                                                    <form action="{{url('/')}}/amb_close_lead" method="post">
                                                        @csrf
                            ...
                            @php
                                ...
                                if(isset($filter_arr['pickup_location'])){
                                    $append_arr['filter_customer_pickup_location'] = $filter_arr['pickup_location'];
                                }
                                if(isset($filter_arr['ambulance_type'])){
                                    $append_arr['filter_ambulance_type'] = $filter_arr['ambulance_type'];
                                }
                                if(isset($filter_arr['drop_location'])){
                                    $append_arr['filter_customer_drop_location'] = $filter_arr['drop_location'];
                                }
...
@section('script')
    @if(request()->routeIs('view_all_ambulance_leads') ? 'active' : '')
        <script>
    ...
    <script>
        ...
        $('#btn_clear').on('click',function(){
            ...
            var url="<?php echo url('/');?>/view_all_ambulance_leads ";
