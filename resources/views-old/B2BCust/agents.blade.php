...
@section('content')
    <div class="card my-3">
        ...
        <div class="card-body border-primary collapse" id="filter-collapse" aria-labelledby="headingTwo" data-parent="#agentsfilter">
            <a id="addnew" name="addnew" value="Add New" class="btn btn-sm btn-outline-primary" href="{{route('agents.create')}}"><i class="fas fa-plus"></i> New</a>
        </div>
    </div>
    <div class="table table-responsive jim-table-responsive">
                        ...
                        <td data-label="Action" >
                            @if($value->flag=='Active')
                                ...
                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton" width="100%">
                                    <a href="{{route('agents.show',$value->id)}}" title="View" class="dropdown-item btn btn-sm btn-outline-primary fas fa-eye text-primary"
                                        data-tooltip="tooltip" data-placement="bottom" title="View">
                                        <span>View</span></a>
                                    <a href="{{route('agents.edit',$value->id)}}" title="Edit" class="dropdown-item btn btn-sm btn-outline-success fas fa-edit  text-success"
                                        data-tooltip="tooltip" data-placement="bottom" title="Edit">
                                        <span>Edit</span>
                                    </a>
                                    <a href="{{route('agents.destroy',$value->id)}}" title="Delete" class="dropdown-item btn btn-sm btn-outline-danger fas fa-trash-alt text-danger"
        ...
        {{$agents->links('Custom.Pagination.pagination')}}
    </div>
@endsection
