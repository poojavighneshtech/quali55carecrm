...
<body id="page-top">	
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <center>
                            ...
                            @csrf
                                <label for="deliverypickup">Order Type</label>
                                ...
                                <div class="row">
                                    <div class="col-md-4"></div>
                                        <div class="col-md-4">
                                            <center><button type="submit" class="form-control btn btn-outline-primary" id="btn_search_customer" name="submit" value="Search">Search</button></center>
                                        </div>
                                </div>
                            </form> --}}
                            <form action="{{url('/')}}/searchCustomerDelReport" method="post">
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <input type="date" class="form-control" name="start_date" id="start_date" value="@if(isset($start_date)){{$start_date}}@endif">
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <input type="date" class="form-control" name="end_date" id="end_date" value="@if(isset($end_date)){{$end_date}}@endif">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <input type="text" class="form-control" name="input_text" id="text_search_customer" placeholder="Search by customer name or number" value="@if(isset($text_customer)){{$text_customer}}@endif">
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="col-md-12">
                                                    <select class="selectpicker form-control" name="deliverypickup" id="deliverypickup">
                                                        <option value="Delivery" @if($deliverypickup=="Delivery") {{'selected'}} @endif>Delivery</option>
                                                        <option value="Collection" @if($deliverypickup=="Collection") {{'selected'}} @endif>Collection</option>
                                                        <option value="Pick Up" @if($deliverypickup=="Pick Up") {{'selected'}} @endif>Pick Up</option>
                                                        <option value="All" @if($deliverypickup=="All") {{'selected'}} @endif>All</option>
                                    ...
                                    <div class="col-md-4">
                                        <div class="row">
                                            <div class="col-md-6 text-center">
                                                <button type="reset" class="form-control btn btn-outline-primary" id="btn_search_customer" name="reset" value="Clear">Reset</button>
                                            </div>
                                        </div>
                                        <br>
                                        <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <span><b>Total Collection : </b></span><span>{{ $total_collection ?? '' }}</span>
                                        <span><b>Cash Collection : </b></span><span>{{ $cash_collection ?? '' }}</span>
                                        <span><b>Online Collection : </b></span><span>{{ $online_collection ?? '' }}</span>
                                    </div>
                                </div>
                                <hr>
                            </form>
                        ...
                        <table id="records" class="table table-bordered table-responsive">
                            ...
                            <tbody>
                                ...
                                <?php
                                    ...
                                    {
                                ?>
                                        <tr class="row_scroll <?php if($order_detail['deliverypickup'] == 'Delivery'){echo "table-success";}elseif($order_detail['deliverypickup'] == 'Pick Up'){echo "table-danger";}elseif($order_detail['deliverypickup'] == 'Collection'){echo "table-warning";}?>">
                                            <td>{{$i}}</td>
                                            ...
                                            <td>{{ date('d-m-Y',strtotime($order_detail['DelDate']))}}</td>
                                            ...
                                            <td>
                                            {{-- <td>@if(isset($order_detail['reference_image']))
                                                    @if($order_detail['reference_image'] != null && $order_detail['reference_image'] != "-")
                                                        <a href="{{url('/').'/assets/uploads/payment_images/'.$order_detail['reference_image']}}" target="_blank">View Image</a>
                                                    @else
                                                        -
                                                    @endif
                                                @endif --}}
                                                @if(isset($order_detail['payment_image']))
                                                    @if($order_detail['payment_image'] != null && $order_detail['payment_image'] != "-")
                                                        <a href="{{url('/').'/assets/uploads/payment_images/'.$order_detail['payment_image']}}" target="_blank">View Image</a>
                                                    @else
                                                        -
                                                    @endif
                                                @else
                                            ...
                                            <td>                                                
                                                @if($order_detail['cust_sign'] != null || $order_detail['cust_sign'] != "")
                                                    
                                                    <a href="http://{{ $order_detail['cust_sign']}}" target="_blank">View Image</a> 
                                                    
                                                @else
                                                    
                                                    {{$order_detail['cust_sign']}}
                                                    
                                                @endif
                                            </td>
                                            <td>
                                                {{-- {{ $order_detail['product_delivered']}} --}}
                                                @php
                                                    $product_delivered = json_decode($order_detail['product_delivered']);
                                                    if($product_delivered != null || $product_delivered != "")
                                                    {
                                                        $count = count($product_delivered);
                                                    }
                                                    else
                                                    {
                                                        $count = 0;
                                                ...
                                                @endphp
                                                    @if($count >0)
                                                        @foreach($product_delivered as $key=>$value)
                                                        <a href="http://{{$value}}" target="_blank">View Image {{$key+1}}</a>
                                                        @endforeach
                                                    @else
                                                        {{$order_detail['product_delivered']}}
                                                    @endif 
    ...
    <script>
        $(document).ready(function(){
            $('#deliverypickup').selectpicker('render');
