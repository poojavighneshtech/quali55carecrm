        ...
        <div class="leads">
            ...
            <div class="card">
                <div class="card-header"  style="background-color: #4a6fdc; color: white;" >
                    <center>All Lab Test Leads</center>
                </div> 
                ...
                <div class="card-body">
                    <div class="card" id="filter_card">
                        ...
                        <div class="card-body collapse @if(isset($filter_arr) && !array_filter($filter_arr)) hide @else show @endif" id="filter-collapse" aria-labelledby="heading-filter">
                            <form action="{{url('/')}}/view_all_lab_test_leads" method="GET" id="all_lab_leads_form">
                                @csrf
                                <div class="row">
                                    <div class="col-md-10">
                                        <div class="row">
                                            <div class="col-md-7">
                                                ...
                                                <div class="row">
                                                    <div class="col-md-4 text-right">
                                                        <label for="lead source"><strong>Lab Name :</strong></label>
                                                    </div>
                                                    <div class="col-md-8 text-right">
                                                        <select class="select form-control selectpicker" name="filter_lab_name" id="select_filter_lab_name" title="Lead source"
                                                            data-live-search="true" data-size="3">
                                                            <option value="All" selected>All</option>
                                                            @foreach($get_lab_names as $lab)
                                                                <option value="{{$lab->id}}" @if(isset($filter_arr['lab_id']) && $lab->id==$filter_arr['lab_id'])selected @endif>{{$lab->lab_name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="row">
                                <div class="table">
                                    <table class="table table-hover table-responsive" id="tbl_view_all_leads">
                                        <thead>
                                            <tr>
                                                <th>Sr No</th>
                                        ...
                                        <tbody>
                                            @foreach($get_all_leads as $key=> $lead) 
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span style="display:none">{{date('Y-M-d',strtotime($lead->created_date))}}</span>
                                                        {{date('d-M-Y',strtotime($lead->created_date))}}
                                                    </td>
                                                    <td>{{$lead->customer_name}}</td>
                                                    <td>{{$lead->contact_no}}</td>
                                                    <td>{{$lead->location}}</td>
                                                    <td>{{$all_leads_arr['data'][$key]['test_names']}}</td>
                                                    <td>{{$lead->visit_date}}</td>
                                                    <td>{{$lead->visit_time}}</td>
                                                    <td>{{$lead->l_name}}</td>
                                                    <td>{{$lead->customer_price}}</td>
                                                    ...
                                                    <td>
                                                        @if($lead->status=='In Process')
                                                            <button type="button" class="btn btn-outline-success" data-toggle="modal" data-target="#convert_modal"
                                                                data-tooltip="tooltip" data-placement="bottom" title="Convert Lead">
                                                                <i class="far fa-check-circle"></i>
                                                            </button>
                                                            <form action="{{url('/')}}/lab_convert_lead" method="post">
                                                                @csrf
                                                                    ...
                                                                    <div class="modal-dialog" role="document">
                                                                        <div class="modal-content">
                                                                            ...
                                                                            <div class="modal-body">
                                                                                ...
                                                                                <input type="hidden" name="lead_id" id="modal_txt_lead_id" value="{{$lead->lead_id}}">
                                                                            </div>
                                                            ...
                                                            <button type="button" class="btn btn-outline-danger" data-toggle="modal" data-target="#close_modal"
                                                                data-tooltip="tooltip" data-placement="bottom" title="Close Lead">
                                                                <i class="far fa-window-close"></i>
                                                            </button>
                                                            <form action="{{url('/')}}/lab_close_lead" method="post">
                                                                @csrf
                                    ...
                                    @php
                                        ...
                                        if(isset($filter_arr['lab_id'])){
                                            $append_arr['filter_lab_name'] = $filter_arr['lab_id'];
                                        }
...
@section('script')
    @if(request()->routeIs('view_all_lab_test_leads') ? 'active' : '')
        <script>
    ...
    <script>
        ...
        $('#btn_clear').on('click',function(){
            ...
            var url="<?php echo url('/');?>/view_all_lab_test_leads ";
