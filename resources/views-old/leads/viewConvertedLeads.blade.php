        ...
        <div class="leads">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    {{-- <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <input type="date" class="form-control" name="start_date" id="start_date" value="<?php if(isset($data['lead_details'])){echo "dasddad".$data['lead_details'][0]['start_date'];}?>">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 ">
                    ...
                    <div class="row">
                        <div class="col-md-2">
                            <a class="btn btn-primary btn-sm btn-block" href="<?php echo url('/')?>/create_lead">Create New Lead</a>
                        </div>
                        <div class="col-md-2">
                            <button type="button" id="export" class="btn btn-primary btn-sm btn-block">Export To Excel</button>
                        </div>
                        <div class="col-md-8">
                            <div class="row">
                    ...
                    <div class="table">
                        <table id="records" class="table table-striped table-responsive" id="records">
                            <thead>
                                ...
                                <th>Customer Name</th>
                                <th>Mobile Number</th>
                                <th>Equipment &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</th>
                                <th>Location</th>
                                <th>Status</th>
                                <th>Lead Owner</th>
                                ...
                                <?php
                                    $count = 0;
                                        ...
                                        foreach($lead_details as $lead_detail)
                                        {
                                            ...
                                            <?php
                                            echo "<td>".$sr_no."</td>";
                                            echo "<td>".date('d-M-Y',strtotime($lead_detail['converted_at']))."</td>";
                                            ?>
                                            <td>
                                                <div class="more">
                                                    {{substr($lead_detail['comment'],'0','20')}}
                                                </div>
                                        ...
                                        <?php 
                                            ...
                                            ?>
                                                ...
                                                <td>
                                                    @if($lead_detail['delivery_challan']!=null)
                                                        <button href="#sent_email_popup" class="btn btn-sm btn-outline-success"  data-toggle="modal" 
                                                                    data-email-id="{{$lead_detail['email_id']}}" 
                                                                    data-lead-id="{{$lead_detail['id']}}" 
                                                                    data-cust-id="{{$lead_detail['customer_id']}}" 
                                                                    data-challan-id="{{url('/').$lead_detail['delivery_challan']}}" data-toggle="tooltip" data-placement="top" title="view and send challan to customer">
                    ...
                    {{--Sent email popup --}}
                    <div class="modal" id="sent_email_popup">
                        <div class="modal-dialog">
                            <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <form class="form" method="post" action="{{url('/')}}/sent_challan">
                                {{csrf_field()}}
                                <div class="modal-body">
                                    {{-- <input type="text" class="form-control" name="modal_email_id"> --}}
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text" id="inputGroup-sizing-default">Email ID</span>
                                        </div>
                                        <input type="text" class="form-control" aria-label="Default" aria-describedby="inputGroup-sizing-default" name="modal_email_id">
                                        </div>
                                    <br>
                                    <embed id="modal_chllan_id" name="modal_chllan_id"
                                        frameborder="0" width="100%" height="400px">
                                    <input type="hidden" name="modal_lead_id" >
                                    <input type="hidden" name="modal_cust_id" >
                                </div>
                                <div class="modal-footer">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#export').on('click', function(){
            $('<table>').append(table.$('tr').clone()).table2excel({
                //exclude: ".excludeThisClass",
                //name: "abc",
                filename: "AllConvertedLeads-{{date('d-m-Y')}}"
            });
        });
        $("#search").on('input',function(){
            var search = $("#search").val();
        ...
        $('#filter').on("change",function(){
            var filter_by = $('#filter').val();
            var section = "Converted";
            localStorage['filtered'] = filter_by;
            //alert(filter_by);
            var dataString = (filter_by);
            var url = "<?php echo url('/');?>/filterConvertedLeads/"+dataString;
            window.location.assign(url);
        });
    ...
    $('#sent_email_popup').on('show.bs.modal', function(e) {
        var EmailId = $(e.relatedTarget).data('email-id');
        $(e.currentTarget).find('input[name="modal_email_id"]').val(EmailId);
        var LeadId = $(e.relatedTarget).data('lead-id');
        $(e.currentTarget).find('input[name="modal_lead_id"]').val(LeadId);
        var CustId = $(e.relatedTarget).data('cust-id');
        $(e.currentTarget).find('input[name="modal_cust_id"]').val(CustId);
        var Challan_Path = $(e.relatedTarget).data('challan-id');
        var game=document.getElementById("modal_chllan_id");
        var clone=game.cloneNode(true);
        clone.setAttribute('src',Challan_Path);
        game.parentNode.replaceChild(clone,game)
        // $(e.currentTarget).find('embed[name="modal_chllan_id"]').attr('src', Challan_Path);
        // $('#modal_chllan_id').attr('src', null);
        // $('#modal_chllan_id').attr('src', Challan_Path);
    });
