    ...
    @section('content')
        ...
        @endif 
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{url('/')}}/filterCollectionOrder" method="post">
                        @csrf
                        <div class="row">
                            <div class="col-md-2">
                        ...
                        <table class="table table-bordered" id="records">
                            ...
                            <tbody>
                                ...
                                @foreach($collection_request as $collection_order)
                                    <tr>
                                        <td>{{$srno}}</td>
                                        <td data-label="Collection Date">{{date('d-m-Y',strtotime($collection_order['DelDate']))}}</td>
                                        <td class="text-nowrap" data-label="Created on">{{date('d-M-y h:i',strtotime($collection_order['created_at']))}}</td>
                                        <td data-label="Order Id">{{$collection_order['order_id']}}</td>
                                        <td data-label="Customer Name">{{$collection_order['shipping_first_name']}}</td>
                                        <td data-label="Mobile No">{{$collection_order['mobileno']}}</td>
                                        <td data-label="Products">{{$collection_order['line_item_1']}}</td>
