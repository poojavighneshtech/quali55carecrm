...
<head>
    ...
    <title>In Progress Referrer</title>
            ...
            <div class="container">  
                <div class="card">
                    <div class="card-header"  style="background-color: #4a6fdc; color: white;" >
                        <center>In-Progress Referrer</center>
                    </div> 
                    <div class="card-body">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($nurses_data as $nurse_data)
                                        ...
                                        <tr data-count="{{$count}}">
                                            <td>{{$count}}</td>
                                            <td>
                                                <a class="btn btn-success btn-sm btn-block" href="{{url('/')}}/view_nurse_details/{{$nurse_data['id']}}" role="button">Referrer it</a> 
                                                {{-- <a class="btn btn-primary btn-sm btn-block" href="{{url('/')}}/in_progress/{{$nurse_data['id']}}/{{session('user_id')}}" role="button">Progress it</a>  --}}
                                                <a class="btn btn-danger btn-sm btn-block" href="#" role="button" data-toggle="modal" data-target="#myModal" title="Close">Close</a> 

                                                {{--hidden vvalues--}}
                                                <input type="hidden" name="nurse_id{{$count}}" id="nurse_id{{$count}}" value="{{$nurse_data['id']}}">
                                                <input type="hidden" name="user_id{{$count}}" id="user_id{{$count}}" value="{{session('user_id')}}">
                                            </td>
                                            <td>
                                                <div class="more">
                                                    {{$nurse_data['comment']}}
                      ...
                      <div class="modal-content">
                        ...
                        <form action="">
                            ...
                            <div class="modal-body">
                                {{-- <input type="hidden" name="modal_nurse_id" id="modal_nurse_id" value=""> --}}
                            ...
                            <div class="modal-footer">
                                <a class="btn btn-secondary" id="close_nurse" onclick="getlink();" href="#" {{--style="pointer-events: none; color: #ccc;"--}} title="Close"><i class="fas fa-window-close"></i> Close lead</a>
     ...
     function getlink()
        {
            ...
            var desc = $('#desc').val();
            document.getElementById('close_nurse').href += '/'+reason+'/'+desc;
        }
        ...
        $('#records tr').click(function() {    
            ...
            //var desc = $('#desc').val();
            document.getElementById('close_nurse').href ="<?php echo url('/');?>/close_nurse/"+user_id+"/"+nurse_id;
