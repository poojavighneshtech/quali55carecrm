        ...
        {{ csrf_field() }}
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Rejected Rents</h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-9">
                                ...
                                @if($vendor_product_counts !=null)
                                    @foreach ($vendor_product_counts as $vendor_product_detail) 
                                        <tr  data-toggle="collapse" data-target="#demo{{$i}}" class="data-toggle" data-id="{{$i}}">
                                            ...
                                            <td data-label="Rejected Products"><b>{{$vendor_product_detail['count']}}</b></td>
                                        </tr>
                                        <tr>
                                                        ...
                                                        <tbody>
                                                            @for($j = 0; $j <count($vendor_product_detail['products']); $j++)
                                                                <tr class="table-light">
                                                                    ...
                                                                    <td data-label="Rent Rejected">{{$vendor_product_detail['products'][$j]['product_rent_rejected']}}</td>
                                                                </tr>
                                                            @endfor
                                                        </tbody>
                                                    </table>
...
<script>
    $(document).ready(function () {
        ...
        $('#city_filter').on("change",function(){
            ...
            var url = "<?php echo url('/');?>/product_rejected_rent/"+dataString;
